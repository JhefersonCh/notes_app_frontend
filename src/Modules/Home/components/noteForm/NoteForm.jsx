...
export const NoteForm = ({
  ...
  const tagsArray = currentNote?.tags?.length
    ? currentNote?.tags.split(',')
    : [];
  ...
  useEffect(() => {
    ...
    setTags(
      tagsArray ? tagsArray.map((tag) => ({ label: tag, value: tag })) : [],
    );
  }, [currentNote]);
  ...
  const updateDisabledOptions = (selectedTags) => {
    const updatedOptions = TAG_OPTIONS.map((option) => {
      ...
      const isDisabled = contradictedTags.some((tag) =>
        selectedTags.some((selected) => selected.value === tag),
      );
    ...
    <div className="note-form-container">
      ...
      <form className="d-flex flex-column gap-2">
        <div className="form-field-login">
          <label htmlFor="title" className="input-label mb-3">
            Title
          </label>
        ...
        <div className="form-field-login position-relative">
          <label htmlFor="tags" className="input-label mb-3">
            Tags
          </label>
          <MultiSelect
        ...
        <div className="form-field-login position-relative">
          <label htmlFor="content" className="input-label mb-3">
            Content
          </label>
